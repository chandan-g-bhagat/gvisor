...
// cpuSet represents the map of all CPUs on the system, mapped
// by Physical ID and CoreIDs.
type cpuSet map[string]*threadGroup
...
type thread struct {
	...
	physicalID      int64               // This CPU's physical id to match Hyperthread Pairs.
...
`
	...
	return fmt.Sprintf(template, t.processorNumber, t.physicalID, t.coreID, t.vendorID, t.cpuFamily, t.model, strings.Join(bugs, ","))
}

// ID returns an ID for this thread, which is its physical and core ID.
func (t *thread) ID() string {
	return fmt.Sprintf("%d:%d", t.physicalID, t.coreID)
}
